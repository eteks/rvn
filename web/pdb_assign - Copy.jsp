...
<%@include file="sidebar.jsp" %>
                                                                       ...
                                                                       <td class="">
                                                                        ...
                                                                        <td class="text-center" ng-repeat="i in records"> 
                                                                            ...
                                                                            <label class="custom_radio mytooltip tooltip-effect-8">                                                                                
                                                                                <input type="radio" ng-click="radiovalue(record.fid,i.vehicle_model_mapping_id,'y')" ng-model="duno.name" name="f{{record.fid}}_{{i.vehicle_model_mapping_id}}" value="y" >
                                                                                <span class="checkmark c_b_g"></span>
                                                                                <span class="tooltip-content2">yes</span>
                                                                            </label>
                                                                            
                                                                            <label class="custom_radio mytooltip tooltip-effect-8">
                                                                                <input type="radio" ng-click="radiovalue(record.fid,i.vehicle_model_mapping_id,'n')" ng-model="duno.name" name="f{{record.fid}}_{{i.vehicle_model_mapping_id}}" value="n">
                                                                                <span class="checkmark c_b_r"></span>
                                                                                <span class="tooltip-content2">no</span>
                                                                            </label>
                                                                            
                                                                            <label class="custom_radio mytooltip tooltip-effect-8">
                                                                                <input type="radio" ng-click="radiovalue(record.fid,i.vehicle_model_mapping_id,'o')" ng-model="duno.name" name="f{{record.fid}}_{{i.vehicle_model_mapping_id}}" value="o">    
            ...
            <div id="modal-feature-list" class="modal modal-feature-list">
                <div class="modal-content">
                    <h5 class="text-c-red m-b-10">
                        Feature <a class="modal-action modal-close waves-effect waves-light float-right m-t-5" >
                        <i class="icofont icofont-ui-close"></i></a>
                    </h5>                    
                    <ul>
                        <li ng-repeat="fil in features_list">
                            <a href="#" class="text-c-green" ng-click="add_feature_tab(fil.fid)">
                                <i class="icofont icofont-ui-add"></i>
                            </a>&nbsp;({{fil.domain}})&nbsp;{{fil.fea}}
                        </li>
                    </ul>                    
                </div>
            </div>
...
//                alert($scope.data.pdbversion);
                ...
                .then(function (response, status, headers, config){
                    ...
                    var vehicledetail=result_data.vehicledetail_list[0].modelname;
//                    var pdbdetail=result_data.featuredetail_list;
                    var pdbdetail=[{"fid":1,"featurename":"feature1","domainname":"domain1","vmm_id":"1,2","dfm_id":"1","status":"y,y"},
                     {"fid":2,"featurename":"feature2","domainname":"domain1","vmm_id":"1,2","dfm_id":"2","status":"n,o"}];
//                        vehicledetail=vehicledetail.slice(1, -1); 
//                        alert();
                        for(var j=0; j<vehicledetail.split(",").length; j++)
                        {
//                            $scope.records.push({mod:vehicledetail.split(",")[j],vehicle_model_mapping_id:vehicledetail.split(",")[j]});
                              $scope.records.push({mod:vehicledetail.split(",")[j]});
                        }
                        for(var i=0; i < pdbdetail.length; i++)
                        {
                            $scope.features.push({fid:pdbdetail[i].dfm_id,fea:pdbdetail[i].featurename,domain:pdbdetail[i].domainname});
                             
                            for(var k=0; k < pdbdetail[i].vmm_id.split(",").length; k++)
                            {
                                //$scope.records.push({mod:vehicledetail.split(",")[j]});
                                $scope.list.push({vmm_id:pdbdetail[i].vmm_id.split(",")[k],dfm_id:pdbdetail[i].dfm_id,status:pdbdetail[i].status.split(",")[k]});
//                                var name =  "name_" + pdbdetail[i].dfm_id + "_"+pdbdetail[i].vmm_id.split(",")[k];
                                $scope.duno = {
                                name: pdbdetail[i].status.split(",")[k]
                              }
                            }//                            alert(pdbdetail[i].vmm_id.split(",").length);
                        }
